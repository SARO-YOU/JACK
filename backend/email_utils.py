...
# ── Gmail credentials (reads from .env via os.environ) ────────────────────────
...
def send_email(to_email, subject, body, html=None):
    """Send email using Gmail SMTP — completely free, no API key needed."""
    try:
        if not to_email:
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="padding:32px;">
          ...
          <p style="color:#555;font-size:15px;line-height:1.6;margin:0 0 20px;">Your account has been successfully created. You can now browse our fresh products and place orders delivered right to your door.</p>
        ...
        <tr><td style="background:#f8f9fa;padding:16px 32px;text-align:center;border-top:1px solid #e2e8f0;">
          <p style="color:#94a3b8;font-size:12px;margin:0;">Copyright {datetime.utcnow().year} Noory Shop</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
# ═══════════════════════════════════════════════════════════════════════════════
# ORDER CONFIRMATION  
# Called from app.py as: send_order_confirmation(email, name, order_id, total)
# Extended signature supports full itemised receipt
# ═══════════════════════════════════════════════════════════════════════════════
...
def send_order_confirmation(user_email, user_name, order_id, total_price,
    ...
    if order_items:
        ...
        items_html = """<tr><td colspan="3" style="padding:14px;font-size:14px;color:#888;text-align:center;">See your account for full item details</td></tr>"""
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="padding:28px 32px 0;">
          ...
          <p style="font-size:14px;color:#666;line-height:1.6;margin:0;">
            Thank you for your order! We have received it and your items are being prepared for delivery.
          </p>
        </td></tr>
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <thead>
                <tr style="background:#1a1a2e;">
                  <th style="padding:12px 14px;text-align:left;font-size:11px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Item</th>
                  <th style="padding:12px 14px;text-align:center;font-size:11px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Qty</th>
                  <th style="padding:12px 14px;text-align:right;font-size:11px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Total</th>
                </tr>
              </thead>
              <tbody>
                {items_html}
        ...
        <tr><td style="background:#f8f9fa;padding:18px 32px;text-align:center;border-top:1px solid #e2e8f0;">
          ...
          <p style="font-size:12px;color:#94a3b8;margin:0;">Copyright {year} Noory Shop</p>
        </td></tr>
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="padding:24px 32px;">
          ...
          <p style="font-size:14px;color:#555;line-height:1.6;margin:0 0 18px;">A new delivery order is waiting for you. Log into your dashboard to accept it!</p>
          <div style="background:#f8f9fa;border-radius:12px;padding:16px;margin-bottom:14px;">
            <p style="font-size:11px;font-weight:700;color:#888;text-transform:uppercase;margin:0 0 10px;">Delivery Info</p>
