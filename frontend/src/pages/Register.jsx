  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ background: 'linear-gradient(160deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '60px 40px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'url("https://images.unsplash.com/photo-1619566636858-adf3ef46400b?w=800&fit=crop") center/cover', opacity: 0.12 }} />
      ...
      {/* RIGHT PANEL */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', padding: '40px', overflowY: 'auto' }}>
        <div style={{ width: '100%', maxWidth: '420px', paddingTop: '20px' }}>

          <div style={{ marginBottom: '28px' }}>
            <h2 style={{ fontSize: '30px', fontWeight: '900', color: '#1a1a2e', margin: '0 0 8px', letterSpacing: '-1px' }}>Create your account ✨</h2>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showPass ? 'text' : 'password'} value={form.password}
                    ...
                    placeholder="Min 6 characters" style={{ ...inp, paddingRight: '48px' }}
                    onFocus={e => { e.target.style.borderColor = '#e94560'; e.target.style.background = '#fff'; }}
                    onBlur={e => { e.target.style.borderColor = '#e2e8f0'; e.target.style.background = '#f8f9fa'; }} />
            ...
            <button type="submit" disabled={loading}
              style={{ width: '100%', padding: '15px', background: loading ? '#ccc' : '#e94560', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '800', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', marginTop: '20px', boxShadow: loading ? 'none' : '0 4px 15px rgba(233,69,96,0.4)', transition: 'all 0.2s' }}>
          ...
          <p style={{ textAlign: 'center', color: '#888', fontSize: '14px', marginTop: '20px', fontWeight: '500' }}>
            ...
            <span onClick={() => navigate('/login')} style={{ color: '#e94560', fontWeight: '800', cursor: 'pointer', textDecoration: 'underline' }}>
              Login! 🔑
            </span>
          </p>
          ...
          <div style={{ marginTop: '16px', background: '#f0fdf4', border: '2px solid #bbf7d0', borderRadius: '12px', padding: '14px 16px', textAlign: 'center' }}>
            <p style={{ margin: 0, fontSize: '13px', color: '#16a34a', fontWeight: '700' }}>
              ...
              <span onClick={() => navigate('/driver-apply')} style={{ cursor: 'pointer', textDecoration: 'underline', fontWeight: '800' }}>
                Apply as a Driver →
              </span>
            </p>
          </div>
          ...
          <div style={{ marginTop: '20px', textAlign: 'center' }}>
            <button onClick={() => navigate('/')} style={{ background: 'transparent', border: 'none', color: '#aaa', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
              ← Back to Shop
            </button>
          </div>
        </div>
